...
module.exports = {
  ...
  validateInfo: async (name, email, password, isAdmin)=> {
   if (typeof(name) === "string" && emailRegex.test(email) && typeof(password) === "string" && (isAdmin === "true" || isAdmin === "false")){
      return true
   }else{return false }
   },
   ...
   tokenGenerator:async(email)=>{
      const userData = await users.findOne({attributes:['isAdmin'],where:{email}})
      const isAdmin = userData.isAdmin
      const token = await jwt.sign({email,isAdmin}, KEY, { expiresIn : '1h'});
      return token
   }
